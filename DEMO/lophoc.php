...
<head>
    ...
    <style>
        ...
        /* Đặt màu nền cho dòng vừa cập nhật */
        .highlighted-row {
            ...
            <div class="col-sm-9"><br>
                ...
                <div class="table-container mt-3">
                    ...
                    if ($result->num_rows > 0) {
                        // Lấy mã lớp vừa cập nhật để tô màu nền
                        $highlightedClass = $_SESSION['updated_class'] ?? null;

                        echo "<table class='table table-hover'>";
                        echo "<tr><th>Mã lớp</th><th>Tên lớp</th><th>Ghi chú</th><th>Danh sách lớp</th></tr>";

                        while ($row = $result->fetch_assoc()) {
                            // Kiểm tra và gán lớp 'highlighted-row' cho dòng vừa cập nhật
                            $highlightClass = ($row['maLop'] === $highlightedClass) ? "class='highlighted-row'" : "";
                            ...
                            echo "<td><a href='dssv_theo_lop.php?maLop=" . htmlspecialchars($row['maLop']) . "' class='btn btn-link'>Xem sinh viên</a></td>";
                            echo "</tr>";
                        }
                        echo "</table>";

                        // Xóa mã lớp đã cập nhật khỏi session sau khi hiển thị để không tô màu lại lần sau
                        unset($_SESSION['updated_class']);
                    } else {
                        echo "<div class='alert alert-warning'>Không có lớp nào.</div>";
                    }
                    $conn->close();
                    ?>
                </div>
