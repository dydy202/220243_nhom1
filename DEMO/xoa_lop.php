...
if (isset($_GET['maLop'])) {
    ...
    // Kiểm tra xem có sinh viên trong lớp này hay không
    ...
    if ($row['count'] > 0) {
        ...
        $stmt->bind_param("s", $maLop);
        $stmt->execute();
        $message = "<div class='alert alert-success'>Lớp đã được chuyển vào thùng rác.</div>";
    } else {
        $deleteSql = "DELETE FROM lophoc WHERE maLop = ?";
        $stmt = $conn->prepare($deleteSql);
        $stmt->bind_param("s", $maLop);
        $stmt->execute();
        $message = "<div class='alert alert-success'>Lớp đã được xóa thành công.</div>";
    }
}

// Truy vấn danh sách lớp chưa xóa
    ...
    <script>
        function confirmDelete(maLop) {
            if (confirm("Bạn có chắc chắn muốn xóa lớp này?")) {
                window.location.href = 'xoa_lop.php?maLop=' + maLop;
            }
        }
    </script>
</head>
    ...
    <div class="container-fluid">
        <div class="row content">
            ...
            <div class="col-sm-9"><br>
                ...
                // Viết câu truy vấn, chỉ lấy lớp chưa xóa
                $sql = "SELECT * FROM lophoc WHERE is_deleted = 0";
                $result = $conn->query($sql);
                ?>
                ...
                <div class="table-container mt-3">
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>
                                    <a href='javascript:void(0);' onclick='confirmDelete(\"" . htmlspecialchars($row['maLop']) . "\")' class='btn btn-link'>Xóa</a></td>"; // Gọi hàm xác nhận
                            echo "</tr>";
                        }
