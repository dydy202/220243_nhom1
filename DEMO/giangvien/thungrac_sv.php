    ...
    <div class="container-fluid">
        <div class="row content">
            ...
            <div class="col-sm-9"><br>
                ...
                // Truy vấn lấy các sinh viên đã bị đánh dấu là đã xóa
                $sql = "SELECT * FROM sinhvien WHERE is_deleted = 1";
                ...
                <div style="display: flex; justify-content: center; align-items: center; width: 100%; margin: 0 auto;">
                    <h2>Thùng rác - Sinh viên đã xóa</h2>
                </div>
                ...
                <div class="table-container mt-3">
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        echo "<tr><th>Mã sinh viên</th><th>Họ</th><th>Tên sinh viên</th><th>Lớp</th><th>Thao tác</th></tr>";
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>
                    <a href='javascript:void(0);' onclick='confirmRestore(\"" . htmlspecialchars($row['maSV']) . "\")' class='btn btn-link'>Khôi phục</a> | 
                    <a href='javascript:void(0);' onclick='confirmDelete(\"" . htmlspecialchars($row['maSV']) . "\")' class='btn btn-link'>Xóa vĩnh viễn</a>
                  </td>";
                            echo "</tr>";
                        }
                        ...
                        echo "<div class='alert alert-warning'>Không có sinh viên nào trong thùng rác.</div>";
                    }
                    ...
                    if (isset($_GET['maSV'])) {
                        ...
                        if ($action === 'restore') {
                            // Khôi phục sinh viên bằng cách đặt is_deleted = 0
                            $restoreSql = "UPDATE sinhvien SET is_deleted = 0 WHERE maSV = ?";
                            $stmt = $conn->prepare($restoreSql);
                            $stmt->bind_param("s", $maSV);
                            $stmt->execute();
                            echo "<div class='alert alert-success'>Sinh viên đã được khôi phục thành công.</div>";
                        } elseif ($action === 'delete') {
                            // Xóa sinh viên vĩnh viễn
                            $deleteSql = "DELETE FROM sinhvien WHERE maSV = ?";
                            $stmt = $conn->prepare($deleteSql);
                            $stmt->bind_param("s", $maSV);
                            $stmt->execute();
                            echo "<div class='alert alert-success'>Sinh viên đã được xóa vĩnh viễn.</div>";
                        }
                    }
                ...
                <script>
                    function confirmRestore(maSV) {
                        if (confirm("Bạn có chắc chắn muốn khôi phục sinh viên này?")) {
                            window.location.href = '../giangvien/thungrac_sv.php?maSV=' + maSV + '&action=restore';
                        }
                    }

                    function confirmDelete(maSV) {
                        if (confirm("Bạn có chắc chắn muốn xóa sinh viên này vĩnh viễn?")) {
                            window.location.href = '../giangvien/thungrac_sv.php?maSV=' + maSV + '&action=delete';
                        }
                    }
                </script>
            </div>
